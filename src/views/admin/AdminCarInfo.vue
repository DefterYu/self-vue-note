<template>
    <div class="">
        <el-tabs v-model="activeName" class="demo-tabs" type="card" @tab-change="tabChange">
            <el-tab-pane label="已上架 在售" :name="0" />
        ...
        <el-table :data="state.list" style="width: 100%" empty-text="暂无数据" stripe border>
            ...
            <el-table-column prop="carNumber" label="剩余数量" width="100" />

            <el-table-column fixed="right" label="操作">
                <template #default="scoped">
                    <el-button type="primary" size="small" @click="editRow(scoped.row)">
                        编辑
                    </el-button>
                    <el-button :type="scoped.row.isSales == 0 ? 'warning' : 'success'" size="small"
                        @click="banClick(scoped.row)">
                        {{ scoped.row.isSales == 0 ? '下架' : '上架' }}
                    </el-button>

                    <el-popconfirm title="确定要删除么" @confirm="deletClick(scoped.row.carId)">
                        <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="编辑车辆信息">
        <el-form :model="form">
            <el-form-item label="车辆标题">
                <el-input v-model="form.title" maxlength="16" placeholder="车辆标题" />
            </el-form-item>
            <el-form-item label="车辆备注">
                <el-input v-model="form.remarks" :rows="2" type="textarea" placeholder="车辆备注" />
            </el-form-item>
            <el-form-item label="车辆车型">
                <el-col :span="10">
                    <el-select v-model="form.carType" placeholder="选择计费方式">
                        <el-option v-for="item in options.carTypeList" :key="item.value" :label="item.label"
                            :value="item.value" />
                    </el-select>
                </el-col>
                <el-col :span="3">车辆数量</el-col>
                <el-col :span="10">
                    <el-input-number v-model="form.carNumber" :min="0" :max="1000" step-strictly
                        controls-position="right" />
                </el-col>
            </el-form-item>
            <el-form-item label="计费方式">
                <el-col :span="10">
                    <el-select v-model="form.moneyType" placeholder="选择计费方式">
                        <el-option v-for="item in options.money" :key="item.value" :label="item.label"
                            :value="item.value" />
                    </el-select>
                </el-col>
                <el-col :span="3">车辆费用</el-col>
                <el-col :span="8">
                    <el-input v-model="form.moneyValue" placeholder="费用" />
                </el-col>
            </el-form-item>
        </el-form>
