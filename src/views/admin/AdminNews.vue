<template>
    ...
    <el-table :data="state.list" style="width: 100%" empty-text="暂无数据" height="500" stripe border>
        ...
        <el-table-column fixed="right" label="操作">
            <template #default="scope">
                <el-button type="success" plain v-if="scope.row.statu == 1" @click="statuChange(scope.row.id as number, 0)">
                    开放
                </el-button>
                <el-button type="warning" plain v-if="scope.row.statu == 0" @click="statuChange(scope.row.id as number, 1)">
                    禁用
                </el-button>
                <el-popconfirm title="确定要删除么" @confirm="deletClick(scope.row.id as number)">
                    <template #reference>
