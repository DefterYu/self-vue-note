<template>
    <div class="">
        <el-tabs v-model="activeName" class="demo-tabs" type="card" @tab-change="tabChange">
            <el-tab-pane label="未受理" :name="0" />
            <el-tab-pane label="已受理" :name="1" />
        </el-tabs>
        <el-table :data="state.list" style="width: 100%" empty-text="暂无数据" stripe border>
            <el-table-column prop="detail" label="工单内容" width="200" />
            <el-table-column prop="phone" label="预留手机号" width="180" />
            <el-table-column prop="phone" label="反馈时间" width="180">
            ...
            <el-table-column fixed="right" label="操作">
                <template #default="scope">
                    <el-button v-if="scope.row.statu == 0" type="danger" size="small" @click="banClick(scope.row)">
                        受理
                    </el-button>
                    <el-popconfirm title="确定要删除么" @confirm="deletClick(scope.row.id)">
                        <template #reference>
